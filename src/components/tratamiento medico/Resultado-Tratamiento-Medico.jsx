...
import { faPills, faEdit, faEye } from "@fortawesome/free-solid-svg-icons";
import { motion } from "framer-motion";
import * as XLSX from "xlsx";

const ResultadoTratamientoMedico = () => {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(tratamientos.map(item => ({
      nombre_animal: item.nombre_animal,
      especie: item.especie,
      tratamiento: item.tratamiento,
      fecha_tratamiento: item.fecha_inicio,
      fecha_fin: item.fecha_fin,
      observaciones: item.notas
    })));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Tratamientos Médicos");
    XLSX.writeFile(workbook, "tratamientos_medicos.xlsx");
  };
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setTratamientos(tratamientos.map(item => (item.id === id ? { ...item, ...updatedData } : item)));
      closeModal();
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-green-100 p-8 font-montserrat">
      <motion.div
        className="resultado-tratamiento-container max-w-6xl mx-auto mt-10"
            ...
            {modalContent && (
              <div>
                <h2 className="text-2xl font-bold mb-4">
                  {isEditing ? "Editar Tratamiento" : "Detalles del Tratamiento"}
                </h2>
                {isEditing ? (
                  <div>
                  ...
                  <div>
                    ...
                    <p><strong>Tratamiento:</strong> {modalContent.tratamiento}</p>
                    <p><strong>Fecha Inicio:</strong> {modalContent.fecha_inicio}</p>
                    <p><strong>Fecha Fin:</strong> {modalContent.fecha_fin}</p>
                    <p><strong>Notas:</strong> {modalContent.notas}</p>
...
export default ResultadoTratamientoMedico;
